...
std::string MainContextImpl::GetMainURL(
  ...
  if (command_line->HasSwitch(switches::kUrl)) {
    ...
    // Attempt to load a local home page HTML file if no URL was specified.
    // We support these layouts (first match wins):
    //   <cwd>/assets/pages/home.html
    //   <cwd>/CEF/assets/pages/home.html
    //   <ancestor>/assets/pages/home.html (walking up a few levels from cwd)
    // This makes it robust against differing working directories when
    // launched via Visual Studio (e.g. build output folder) or directly from
    // the CEF root directory.
    auto buildFileUrl = [](const std::filesystem::path& p) -> std::string {
      std::string native = p.u8string();
      for (auto& ch : native) {
        if (ch == '\\') ch = '/';
      }
      if (native.size() > 2 && native[1] == ':') {
        return std::string("file:///") + native;
      } else if (!native.empty() && native[0] == '/') {
        return std::string("file://") + native;
      }
      return std::string("file:///") + native;
    };
    ...
    try {
      const std::string base_dir = GetAppWorkingDirectory();
      std::filesystem::path cwd = std::filesystem::u8path(base_dir);
      std::vector<std::filesystem::path> candidates;

      // Direct assets directory under current working directory.
      candidates.push_back(cwd / "assets" / "pages" / "home.html");
      // Nested under a CEF directory (user's structure).
      candidates.push_back(cwd / "CEF" / "assets" / "pages" / "home.html");

      // Walk up to 5 parent levels looking for assets/pages/home.html.
      std::filesystem::path cur = cwd;
      for (int i = 0; i < 5; ++i) {
        cur = cur.parent_path();
        if (cur.empty()) break;
        candidates.push_back(cur / "assets" / "pages" / "home.html");
        candidates.push_back(cur / "CEF" / "assets" / "pages" / "home.html");
      }
      ...
      for (const auto& candidate : candidates) {
        ...
        if (std::filesystem::exists(candidate, ec) &&
          ...
          main_url = buildFileUrl(candidate);
          break;
        }
      }
    } catch (const std::exception&) {
      // Ignore filesystem errors and fall back to default logic.
    }

    // Preserve original behavior for frameless Views windows if still default.
    if (use_views_ && command_line->HasSwitch(switches::kHideFrame) &&
        main_url == kDefaultUrl) {
